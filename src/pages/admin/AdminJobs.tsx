    ...
    <div>
      ...
      {isNew && (
        <div className="admin-card p-6 mb-6">
          <h3 className="text-sm font-sans font-semibold text-[#FFFFCC] uppercase tracking-wider mb-4">Neues Inserat</h3>
      ...
      {loading ? (
        ...
        <div className="admin-card p-12 text-center"><p className="text-white/30 font-sans">Noch keine Inserate.</p></div>
      ) : (
        <div className="space-y-3">
          {jobs.map((j) => (
            <div key={j.id} className={`admin-card overflow-hidden ${!j.is_active ? 'opacity-50' : ''}`}>
              {editing === j.id ? (
                <div className="p-6">
                  <JobForm form={form} setForm={setForm} saving={saving} onSave={save} onCancel={cancel}
                    reqInput={reqInput} setReqInput={setReqInput} addReq={addReq} removeReq={removeReq} />
                ...
                <div className="flex items-center gap-4 p-4">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => toggleActive(j)} className="p-2 text-white/30 hover:text-[#FFFFCC] transition-colors" title={j.is_active ? 'Deaktivieren' : 'Aktivieren'}>
                      {j.is_active ? <ToggleRight className="w-5 h-5 text-emerald-400" /> : <ToggleLeft className="w-5 h-5" />}
                    </button>
                    <button onClick={() => startEdit(j)} className="p-2 text-white/30 hover:text-[#FFFFCC] transition-colors"><Pencil className="w-4 h-4" /></button>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="md:col-span-2">
          ...
          <input className="admin-input" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="z.B. Bäcker/in" />
        </div>
        <div>
          <label className="block text-xs font-sans text-white/50 uppercase tracking-wider mb-1">Standort</label>
          <input className="admin-input" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="z.B. Langstrasse, Zürich" />
        </div>
        <div>
        ...
        <div>
          ...
          <input className="admin-input" value={form.shift || ''} onChange={(e) => setForm({ ...form, shift: e.target.value })} placeholder="z.B. Nachtschicht" />
        </div>
        <div>
        ...
        <div className="md:col-span-2">
          ...
          <textarea className="admin-input min-h-[80px] resize-y" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Stellenbeschreibung" />
        </div>
      </div>
      ...
      <div>
        ...
        {form.requirements.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-3">
            {form.requirements.map((req, i) => (
              <span key={i} className="inline-flex items-center gap-1 px-3 py-1 bg-[#2C2C00] text-white/60 text-xs font-sans rounded-full border border-white/5">
                {req}
        ...
        <div className="flex gap-2">
          <input className="admin-input flex-1" value={reqInput} onChange={(e) => setReqInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addReq(); } }} placeholder="Anforderung eingeben + Enter" />
          <button type="button" onClick={addReq} className="admin-btn-outline text-xs px-4">+</button>
        </div>
      </div>

      <div className="flex items-center gap-3 pt-2 flex-wrap">
        <button onClick={onSave} disabled={saving || !form.title} className="admin-btn flex items-center gap-2 text-sm px-5 py-2.5 disabled:opacity-50">
