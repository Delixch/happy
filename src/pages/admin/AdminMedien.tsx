    ...
    <div>
        ...
        {TYPES.map((t) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-sans transition-all ${
              activeTab === t.id ? 'bg-[#FFFFCC] text-[#1A1A00] font-semibold' : 'bg-[#2C2C00] text-white/50 border border-white/10 hover:text-white/80'
            }`}
          >
      ...
      {isNew && (
        <div className="admin-card p-6 mb-6">
          <h3 className="text-sm font-sans font-semibold text-[#FFFFCC] uppercase tracking-wider mb-4">Neuer Beitrag</h3>
      ...
      {loading ? (
        ...
        <div className="admin-card p-12 text-center"><p className="text-white/30 font-sans">Keine Beiträge in dieser Kategorie.</p></div>
      ) : (
              ...
              {editing === item.id ? (
                ...
                <div className="flex items-center gap-4 p-4">
                  {item.type === 'tv' && item.url && (
                    <div className="relative w-20 h-12 rounded-lg overflow-hidden flex-shrink-0 bg-[#232300]">
                      <img src={`https://i.ytimg.com/vi/${item.url}/default.jpg`} alt="" className="w-full h-full object-cover opacity-70" />
                      <Play className="absolute inset-0 m-auto w-4 h-4 text-[#FFFFCC]" />
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => startEdit(item)} className="p-2 text-white/30 hover:text-[#FFFFCC] transition-colors"><Pencil className="w-4 h-4" /></button>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input className="admin-input" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Beitragsname" />
        </div>
        <div>
          <label className="block text-xs font-sans text-white/50 uppercase tracking-wider mb-1">Typ</label>
          <select className="admin-input" value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value as MediaType })}>
        ...
        <div className="md:col-span-2">
          ...
          <input className="admin-input" value={form.url || ''} onChange={(e) => setForm({ ...form, url: e.target.value })} placeholder={form.type === 'tv' ? 'z.B. SEpS4LEEjzE' : 'https://...'} />
        </div>
        <div className="md:col-span-2">
          <label className="block text-xs font-sans text-white/50 uppercase tracking-wider mb-1">Beschreibung</label>
          <input className="admin-input" value={form.description || ''} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Optional" />
        </div>
        <div>
